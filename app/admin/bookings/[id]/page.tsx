...
import { EnvironmentOutlined, CalendarOutlined, UserOutlined, PhoneOutlined, MailOutlined, CarOutlined } from '@ant-design/icons';
        ...
        <Section className="min-h-screen bg-surface">
            <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <Link href="/admin/bookings" className="text-primary hover:underline mb-2 block">
                            ← Back to Bookings
                        </Link>
                        <h1 className="text-3xl font-bold text-secondary">Booking Details</h1>
                    </div>
                    <Tag color={statusColors[booking.status]} className="text-lg px-4 py-2">
                ...
                <div className="bg-white rounded-xl shadow-lg p-8 mb-6">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-sm font-semibold">{new Date(booking.createdAt).toLocaleString()}</p>
                        </div>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="pl-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p className="font-semibold">{booking.tripType.replace('-', ' ').toUpperCase()}</p>
                            </div>
                            <div>
                                <p className="text-gray-500">Distance</p>
                                <p className="font-semibold">{booking.distance ? `${booking.distance.toFixed(2)} km` : 'N/A'}</p>
                            </div>
                    ...
                    <div className="mb-6">
                        ...
                        <div className="pl-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                                ...
                                <p className="font-semibold">{booking.selectedVehicle?.toUpperCase() || 'Not Selected'}</p>
                            </div>
                            {booking.baseFare && (
                                <div>
                                    <p className="text-gray-500">Base Fare</p>
                                    <p className="font-semibold">£{booking.baseFare.toFixed(2)}</p>
                                </div>
                            )}
                            {booking.distanceCharge && (
                                <div>
                                    <p className="text-gray-500">Distance Charge</p>
                                    <p className="font-semibold">£{booking.distanceCharge.toFixed(2)}</p>
                                </div>
                            )}
                            {booking.totalPrice && (
                                <div>
